	...
	export default {
		...
		methods: {
			...
			async getCommentsList() {
				if (this.type === "feed") {
					// 获取当前动态评论列表信息
					let res = await this.$api.getFeedComments();
					this.commintsList = res.data;
				} else {
					// 获取当前资讯评论列表信息
					let res = await this.$api.getNewComments();
					this.commintsList = res.data;
				}
			},
			// 点赞逻辑操作
				...
				if(this.type === 'feed'){
					...
					this.commintsList.push({
						...
						user: {
							...
							name: this.oneInfoClone.name,
						},
						created_at: timeFrom(timestemp, "yyyy年mm月dd日")
					})
